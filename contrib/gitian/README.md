...
```bash
cp xolentum/contrib/gitian/gitian-build.py .
```
...
o speed up the build, use `-j 5 --memory 10000` as the first arguments, where `5` is the number of CPU's you llocated to the VM plus one, and 10000 is a little bit less than then the MB's of RAM you allocated. If there is emory corruption on your machine, try to tweak these values. A good rule of thumb is, that Monero currently needs bout 2 GB of RAM per core.
...
[xolentum-project/gitian.sigs](https://github.com/xolentum-project/gitian.sigs/) repository:
...
**Note:** Please ensure your gpg public key is available to check signatures by adding it to the [gitian.sigs/gitian-pubkeys/](https://github.com/xolentum-project/gitian.sigs/tree/master/gitian-pubkeys) directory in a pull request.
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/moneromooo-xolentum/bitmonero -b moneromooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
